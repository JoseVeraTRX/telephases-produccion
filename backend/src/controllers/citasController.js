...
exports.getCitaById = async (req, res) => {
  ...
  try {
    const citaQuery = 'SELECT *, TO_CHAR(fecha_cita, \'YYYY-MM-DD"T"HH24:MI\') as fecha_cita_iso FROM citas WHERE id = $1';
...
exports.createCita = async (req, res) => {
  const { paciente_id, fecha_cita, observaciones_admin, examenes_previstos } = req.body;
...
exports.getMyCitas = async (req, res) => {
  ...
  try {
    const query = `
      SELECT
          c.id AS cita_id, c.fecha_cita, c.observaciones_admin, ec.nombre AS estado_cita,
          STRING_AGG(te.nombre, ', ') AS examenes_previstos,
          CASE 
              WHEN c.fecha_creacion > NOW() - INTERVAL '48 hours' AND c.estado_cita_id = 1 THEN TRUE
              ELSE FALSE 
          END as is_new
      FROM citas c
      JOIN estado_cita ec ON c.estado_cita_id = ec.id
...
exports.confirmMyCita = async (req, res) => {
  const { citaId } = req.params;
  const patientId = req.user.userId;
  try {
    const updateQuery = `
      UPDATE citas 
      SET estado_cita_id = 2, fecha_modificacion = NOW() 
      WHERE id = $1 AND paciente_id = $2 AND estado_cita_id = 1
      RETURNING id;
    `;
    const result = await db.query(updateQuery, [citaId, patientId]);
    if (result.rowCount === 0) {
      return res.status(404).json({ message: 'No se encontró la cita, no te pertenece, o ya no está en estado "Programada".' });
    }
    res.status(200).json({ message: '¡Tu cita ha sido confirmada exitosamente!' });
  } catch (error) {
    console.error('Error al confirmar la cita por el paciente:', error);
    res.status(500).json({ message: 'Error interno del servidor.' });
  }
};
